...
def generate_dipole(shape, z_prjs=(0, 0, 1), vox=(1, 1, 1), shift=True,
    ...
    kx, ky, kz = torch.meshgrid(x / FOVx, y / FOVy, z / FOVz, indexing='ij')
...
def load_volume(path):
    ...
    if path.suffix == '.npy':
        ...
        affine = image.affine
        voxel_size = tuple(float(value) for value in image.header.get_zooms()[:3])
    else:
        raise ValueError('Input must be a .npy, .nii, or .nii.gz volume.')

    array = np.asarray(array, dtype=np.float32)
    while array.ndim > 3 and array.shape[-1] == 1:
        array = array.squeeze(-1)
    if array.ndim != 3:
        raise ValueError(f'Expected a 3-D volume, received shape {array.shape}.')

    tensor = torch.from_numpy(array.copy()).unsqueeze(0).unsqueeze(0)
    return tensor, affine, voxel_size


def save_tensor_as_nii(tensor, path, vox=(1, 1, 1), affine=None):
    path = Path(path)
    if not (path.suffix == '.nii' or path.name.endswith('.nii.gz')):
        path = path.with_suffix('.nii.gz')
    output_affine = np.diag((*vox, 1)) if affine is None else affine
    image = nib.Nifti1Image(tensor.squeeze().detach().cpu().numpy(), output_affine)
    nib.save(image, path)
    return path
